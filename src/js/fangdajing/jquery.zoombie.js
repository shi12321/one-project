...
(function ($) {
	...
	$.bie = function(target, source, img) {
		var outerWidth,
			outerHeight,
			xRatio,
			yRatio,
	...
	$.fn.bie = function (options) {
		return this.each(function () {
			var
			settings = $.extend({}, defaults, options || {}),
			//target will display the image1ed iamge
			target = settings.target || this,
			//source will provide image1 location info (thumbnail)
			source = this,
			img = new Image(),
			$img = $(img),
			mousemove = 'mousemove',
			...
			img.onload = function () {
				var image1bie = $.image1bie(target, source, img);
						...
						function (e) {
							$(document).one('mouseup',
								function () {
									...
									$(document).unbind(mousemove, image1bie.move);
								}
							);

							start(e);

							$(document)[mousemove](image1bie.move);
